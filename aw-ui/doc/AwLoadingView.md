...
- 组件实现：[AwLoadingView.kt](E:/workspace/ASProjects/AutoKs/viewtest/src/main/java/com/answufeng/ui/widget/AwLoadingView.kt)
...
| 样式 | 枚举值 | 说明 |
|---|---|---|
| 圆形 | `CIRCULAR` | Material Design 圆形进度条（默认） |
| 点跳动 | `DOTS` | 三个圆点依次跳动 |
| 水平条 | `HORIZONTAL` | 水平进度条 |
| 花朵旋转 | `FLOWER` | 放射状线条旋转动画 |
| 条形波动 | `BARS` | 三个竖条波动动画 |
...
| `loading_tint` | `color` | `colorPrimary` | 着色颜色 |
...
- `CIRCULAR` 和 `HORIZONTAL` 使用系统 `ProgressBar`，其他样式为自定义动画
- 组件会在 `onAttachedToWindow` 自动恢复动画（如果之前正在运行）
- 组件会在 `onDetachedFromWindow` 自动停止动画
- `tintColor` 为 `null` 时使用主题的 `colorPrimary`
