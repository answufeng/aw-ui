...
class AwBottomTabBar
    ...
    constructor(
        ...
        var indicatorHeight: Float = 3f.dpFloat
            set(value) {
        ...
        var iconSize: Float = 24f.dpFloat
            set(value) {
                val newValue = value.coerceAtLeast(0f)
                if (field == newValue) return
                field = newValue
        ...
        var iconTextGap: Float = 4f.dpFloat
            set(value) {
                val newValue = value.coerceAtLeast(0f)
                if (field == newValue) return
                field = newValue
        ...
        var tabMinWidth: Float = 56f.dpFloat
            set(value) {
                val newValue = value.coerceAtLeast(0f)
                if (field == newValue) return
                field = newValue
        ...
        var rippleColor: Int = ContextCompat.getColor(context, R.color.aw_color_ripple)
            set(value) {
                if (field == value) return
                field = value
        ...
        var tabBarElevation: Float = 8f.dpFloat
            set(value) {
                if (field == value) return
                field = value
        ...
        private fun applyAttributes(ta: TypedArray) {
            ...
            indicatorHeight = ta.getDimension(R.styleable.AwBottomTabBar_indicator_height, 3f.dpFloat)
            ...
            iconSize = ta.getDimension(R.styleable.AwBottomTabBar_tab_icon_size, 24f.dpFloat)
            ...
            iconTextGap = ta.getDimension(R.styleable.AwBottomTabBar_tab_icon_text_gap, 4f.dpFloat)
            tabMargin = ta.getDimension(R.styleable.AwBottomTabBar_tab_margin, 0f)
            tabMinWidth = ta.getDimension(R.styleable.AwBottomTabBar_tab_min_width, 56f.dpFloat)
            ...
            rippleColor = ta.getColor(R.styleable.AwBottomTabBar_ripple_color, ContextCompat.getColor(context, R.color.aw_color_ripple))
            tabBarElevation = ta.getDimension(R.styleable.AwBottomTabBar_tabbar_elevation, 8f.dpFloat)
            ...
            return FrameLayout(context).apply {
                ...
                addView(
                    ...
                    FrameLayout.LayoutParams(
                        ...
                        // 角標（badge）目前以內容容器的 TOP|END 來定位。
                        // 若內容使用 MATCH_PARENT，TOP 會對齊到整列高度，造成 ICON_ONLY/TEXT_ONLY 時角標偏移。
                        FrameLayout.LayoutParams.WRAP_CONTENT,
                        Gravity.CENTER,
                    ),
                )
        ...
        fun setBadgeText(
            ...
            if (text.isNullOrBlank()) {
                ...
                badgeStates[index] = BadgeState(BadgeType.TEXT, text)
            }
            applyBadge(index)
        }
        ...
        private fun applyBadge(index: Int) {
            ...
            textView?.apply {
                ...
                setPadding(badgePadding.toInt(), 0, badgePadding.toInt(), 0)
                layoutParams =
                    (layoutParams as FrameLayout.LayoutParams).also { params ->
        ...
        private fun findFirstViewWithTag(
            view: View,
            tag: String,
        ): View? {
        ...
        private fun <T> shiftMapOnInsert(
            source: Map<Int, T>,
            insertIndex: Int,
        ): Map<Int, T> {
        ...
        private fun <T> shiftMapOnRemove(
            source: Map<Int, T>,
            removeIndex: Int,
        ): Map<Int, T> {
            val result = mutableMapOf<Int, T>()
