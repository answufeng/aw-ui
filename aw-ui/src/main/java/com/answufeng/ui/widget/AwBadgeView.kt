...
class AwBadgeView @JvmOverloads constructor(
    ...
    var badgeTextSizePx: Float
        ...
        set(value) {
            badgeTextPaint.textSize = value.coerceAtLeast(8f.dp())
            requestLayout()
            invalidate()
        }

    var dotSizePx: Float = 8f.dp()
        set(value) {
            field = value.coerceAtLeast(4f.dp())
            requestLayout()
            invalidate()
        }

    var minBadgeHeightPx: Float = 18f.dp()
        set(value) {
    ...
    var horizontalPaddingPx: Float = 6f.dp()
        set(value) {
            field = value.coerceAtLeast(2f.dp())
            requestLayout()
            invalidate()
        }
    ...
    init {
        ...
        dotSizePx = ta.getDimension(R.styleable.AwBadgeView_badge_dotSize, 8f.dp())
        minBadgeHeightPx = ta.getDimension(R.styleable.AwBadgeView_badge_minHeight, 18f.dp())
        horizontalPaddingPx = ta.getDimension(R.styleable.AwBadgeView_badge_horizontalPadding, 6f.dp())
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val (desiredWidth, desiredHeight) = when (mode) {
            Mode.HIDDEN -> 0 to 0
            Mode.DOT -> {
