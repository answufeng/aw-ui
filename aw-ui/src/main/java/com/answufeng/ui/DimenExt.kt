...
fun Resources.dpToPx(dp: Int): Int =
    TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dp.toFloat(), displayMetrics).roundToInt()

/**
 * 使用 [Resources] 将 dp 转为 px（浮点 dp 值），与当前 [android.content.res.Configuration] 一致。
 */
fun Resources.dpToPx(dip: Float): Int =
    TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dip, displayMetrics).roundToInt()

/**
 * 使用 [Resources] 的 scaledDensity 将 sp 转为 px，与系统字体缩放一致。
 */
fun Resources.spToPx(sp: Int): Int =
    TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, sp.toFloat(), displayMetrics).roundToInt()

/**
 * 使用 [Resources] 将 sp 转为 px（浮点 sp 值）。
 */
fun Resources.spToPx(sp: Float): Int =
    TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, sp, displayMetrics).roundToInt()
